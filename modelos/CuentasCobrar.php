...
Class CuentasCobrar
{
	...
	public function listar($fecha_inicio,$fecha_fin,$estado,$idsucursal)
	{
		...
		if($estado == "Todos"){
			...
			$sql="SELECT cc.idcpc,date_format(cc.fecharegistro,'%d/%m/%y') as fecharegistro, v.tipo_comprobante, c.nombre, c.num_documento, cc.deudatotal, cc.abonototal, date_format(cc.fechavencimiento,'%d/%m/%y') as fechavencimiento, cc.idventa 
				FROM venta v 
				INNER JOIN cuentas_por_cobrar cc
		        ON v.idventa = cc.idventa
		        INNER JOIN persona c
	...
	{
		$sql="SELECT cc.iddcpc,cc.iddcpc,cc.montopagado,date_format(cc.fechapago,'%d/%m/%y | %H:%i:%s %p') as fechapago,cc.formapago FROM detalle_cuentas_por_cobrar cc
				WHERE idcpc = '$idcpc'
		        ORDER BY iddcpc asc";
		return ejecutarConsulta($sql);		
	}
